...
一个 Go 实现的多种战法筛选和日线回测 API。内置 `DEMO` 演示标的；支持通过通达信行情协议拉取沪深股票的**前复权日线**并持久化到本地 PostgreSQL，重启后自动加载。
目前聚焦 N 字战法，后续开放多种战法。
...
数据通过 `gotdx` 连接通达信行情服务器获取全量前复权日K，写入本地 PostgreSQL（建表自动完成）。前复权数据会随分红除权整体重算，因此每次同步都全量替换该股票的本地序列，避免新旧复权基准混用。服务地址可用 `XSTOCK_ADDR` 修改（默认 `:8080`）。
